...
#pragma unroll
    for (int i = 0; i < B_LANES; i++) {
        ...
        // All reduce comm buffer update
        if constexpr (std::is_same_v<CB_T, half>) {
            // If fp16 store using atomic addition into comm buffer
            asm volatile("global_atomic_pk_add_f16 %0, %1, off\n\t" : : "v"(&communication_buffer_[i * OP_N / 2]), "v"(x));
        }
        if constexpr (std::is_same_v<CB_T, fp8>) {
            // If fp8 read from recently updated D, convert into fp8, and store normally.
            communication_buffer_[i * OP_N / 2] = __hip_fp8x2_e4m3_fnuz(D_[i * OP_N / 2]).__x;
        }
    }
